...
async function verificarUsuarioAtivo() {
    try {
        ...
        // Após validar e carregar o perfil, renderiza os posts privados do usuário
        carregarPosts();
    ...
    card.innerHTML = `
        ...
        <div class="card-content">
            <div class="card-header">
                <span class="card-date">${new Date(post.created_at).toLocaleDateString('pt-BR')}</span>
            </div>
            <p class="card-caption" style="margin-top: 10px; font-size: 14px; line-height: 1.5; color: #555555; word-wrap: break-word;">${post.caption || ''}</p>
        </div>
    `;
...
async function adicionarPostAGaleria(id) {
    ...
    try {
        ...
        alert("Momento adicionado à galeria!");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnDeletePostMobile) {
        btnDeletePostMobile.addEventListener("click", () => {
            if (postAtivoId) {
                ...
                document.body.style.overflow = "";
                executarExclusaoPost(postAtivoId);
            }
        });
    }
    ...
    if (formPublicar) {
        formPublicar.addEventListener("submit", async (e) => {
            ...
            const inputLegenda = document.getElementById("input-legenda").value.trim();
    ...
    document.addEventListener("click", (e) => {
        if (e.target === logoutModal || e.target === optionsModal || e.target === publicarModal || e.target === galeriaModal || e.target === deleteAvatarModal) {
            logoutModal?.classList.remove("active");
            optionsModal?.classList.remove("active");
            publicarModal?.classList.remove("active");
            galeriaModal?.classList.remove("active");
